...
CREATE TABLE `stocks_extends` (
  ...
  `upper_dist_per` double NOT NULL DEFAULT '0' COMMENT '最高价到ene上轨的距离与轨道间距的百分比',
  `lower_dist_per` double NOT NULL DEFAULT '0' COMMENT '最低价到ene下轨的距离与轨道间距的百分比',
  `ma5` double NOT NULL DEFAULT '0',
  `ma10` double NOT NULL DEFAULT '0',
...
DROP TABLE IF EXISTS `watchall`;
CREATE TABLE `watchall` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `code` varchar(10) NOT NULL DEFAULT '',
  `date` varchar(10) NOT NULL DEFAULT '',
  `now` varchar(10) NOT NULL DEFAULT '',
  `trade` double NOT NULL DEFAULT '0',
  `changepercent` double NOT NULL DEFAULT '0',
  `amt2nmc` double NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`),
  UNIQUE KEY code_date_now (`code`, `date`, `now`)
